 ...
 * @param {Array<string>} tableValues The 2D array to analyze for at least 2 
      rows and 2 columns.
 * @return {boolean} Whether or not the 2D array is a valid representation of a 
      chart with at least 2 rows and 2 columns.
 */
function evaluateSelectionForValidChart(tableValues) {
  if (tableValues.length === 1 && tableValues[0].length === 1) {
    UiFunctions.displayAlert(
      "You must select your values that you'd like to send in a chart!");
    return false;
...
function sendTable() {
  ...
  var isHeader = UiFunctions.displayYesNoChoice(
  ...
  try {
    ...
    sendEmails(HtmlTable, emailProperties.getEmails(), 
      emailProperties.getCC(), emailProperties.getSubject(), bodyText);
  } catch (error) {
    UiFunctions.displayAlert(
      "Encountered an error! Please ensure that addresses in Emails and CC " +
        "are valid."
      );
  };
};
 ...
 * @param {Array<string>} tableValues 2D array of values that will be built 
      as a chart.
 * @param {boolean} header Whether or not the first row is a header, in which 
      case, it will be bolded in the HTML table.
 * @return {string} HTML string of the table. 
 */
function getHTMLTable(tableValues, header) {
  var html =
    "<style>table,td{border: 1px solid black; }</style><table style=" +
  ...
  for (var row = 0; row < tableValues.length; row++) {
    ...
    for (var column = 0; column < tableValues[row].length; column++) {
      if (header && row === 0) {
        html += "<th style=\"border: 1px solid black;\"><b>" + 
          tableValues[row][column] + "</b></th>";
      } else {
        html += "<td style=\"border: 1px solid black;\">" + 
          tableValues[row][column] + "</td>";
      };
    };
    html += "</tr>";
  };
...
function sendEmails(HtmlTable, emails, cc, subject, body) {    
  MailApp.sendEmail(emails, subject, null, {
